/**
 * =========================================================================
 * WFS - Gerador de arquivos Excel (.xlsx) 100% client-side
 * =========================================================================
 * Este utilitário monta um arquivo .xlsx real (OOXML / ZIP) sem depender de
 * bibliotecas externas, para que o build do Cloudflare Pages continue igual.
 *
...
// ---------------------------------------------------------------------
// Escritor de ZIP (método "store", sem compressão — aceito pelo Excel)
// ---------------------------------------------------------------------
interface ZipEntry {
  name: string;
  data: Uint8Array;
...
/**
 * Gera um Blob .xlsx a partir de uma matriz de linhas.
 ...
 *   Todas as strings usam t="inlineStr" com estilo de texto puro travado, garantindo
 *   que nem o Excel nem o sistema receptor alterem datas ou telefones.
 */
export function buildXlsxBlob(rows: CellValue[][], sheetName = 'Solicitações'): Blob {
    ...
    .map((row, rowIndex) => {
      ...
      const cells = row
        .map((value, colIndex) => {
          ...
          // Se for número primitivo e não for cabeçalho
          if (typeof value === 'number' && Number.isFinite(value) && !isHeader) {
            return `<c r="${ref}" s="0"><v>${value}</v></c>`;
          }

          // Todo o resto é tratado e gravado explicitamente como texto puro (inlineStr com numFmtId=49 '@')
